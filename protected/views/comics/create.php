          ...
          $count=count($amigos_avatars);
            if(is_array($amigos_avatars)){
              ...
              foreach ($amigos_avatars as $key => $value) {  
                ...
                echo '<div class="itemThumbnail">'.CHtml::image(Yii::app()->request->baseUrl."/Avatar/".$value['avatar_img'],"amigos_avatars",array('id'=>$value['usuario_id'],'width'=>50,'height'=>168)).'<div>'.CHtml::image('https://graph.facebook.com/'.$value['idFb'].'/picture').'</div></div>'; 
              }
              if($count>12) echo '</div></div><a class="prev"><i class="icon-chevron-left"></i></a><a class="next"><i class="icon-chevron-right"></i></a></div>';//btns pre <a ....
            }
        ?>
